...
function GiftInfoRow({ icon: Icon, label, value, highlight }) {
  return (
    <div className="flex items-start gap-3 rounded-xl border border-gagarin-border/60 bg-white/[0.03] p-4">
      <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-lg bg-gagarin-neon/10">
        <Icon className="h-5 w-5 text-gagarin-neon" />
      </div>
      <div className="min-w-0 flex-1">
        <p className="text-sm text-gray-400">{label}</p>
        <p className={`mt-0.5 break-all text-base font-medium ${highlight ? 'text-gagarin-neon' : 'text-white'}`}>
          {value}
        </p>
      </div>
    </div>
...
export default function GiftPage() {
  ...
  return (
    <div className="relative min-h-screen bg-gagarin-dark bg-grid bg-radial-glow">
      <Helmet>
      ...
      <div className="relative z-10 flex min-h-screen items-center justify-center px-4 py-12">
        <div className="w-full max-w-lg">
          <div className="mb-8 text-center">
            <div className="mx-auto mb-4 flex h-16 w-16 items-center justify-center rounded-2xl border border-gagarin-neon/20 bg-gagarin-neon/10">
              <Gift className="h-8 w-8 text-gagarin-neon" />
            </div>
          ...
          {state.phase === 'loading' && (
            <div className="card-dark flex flex-col items-center py-12 text-center">
              <Loader2 className="mb-4 h-10 w-10 animate-spin text-gagarin-neon" />
          ...
          {state.phase === 'success' && (
            <div className="card-dark space-y-5">
              <div className="text-center">
                <div className="mx-auto mb-3 flex h-14 w-14 items-center justify-center rounded-full bg-gagarin-neon/15">
                  <Shield className="h-7 w-7 text-gagarin-neon" />
                </div>
