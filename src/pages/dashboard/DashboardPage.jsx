...
import { userApi, /* trialApi, */ authApi, AUTH_TOKEN_STORAGE_KEY } from '@services/api';
    ...
    <>
      ...
      <section className="py-20 min-h-screen">
        <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          {/* Tabs */}
          <div className="flex gap-1 mb-8 bg-gagarin-card rounded-xl p-1 border border-gagarin-border">
            {tabs.map((tab) => (
              <button
                key={tab.id}
    ...
    <div className="space-y-6">
            ...
            <div key={slot.key} className="card-dark">
              <div className="flex items-center justify-between gap-3 mb-4">
                <div className="flex items-center gap-3 min-w-0">
                  <div className="w-10 h-10 rounded-xl bg-gagarin-neon/10 flex items-center justify-center flex-shrink-0">
                    <Shield className="w-5 h-5 text-gagarin-neon" />
                  </div>
                  <div className="min-w-0">
                    <div className="text-white font-semibold">{slot.label}</div>
                    <div className={`text-sm ${isActive ? 'text-gagarin-green' : 'text-red-400'}`}>
                ...
                {isActive && url && (
                  <div className="flex gap-2 flex-shrink-0">
                    ...
                    >
                      {copied === slot.key ? (
                        <Check className="w-4 h-4 text-gagarin-green" />
                      ) : (
              ...
              {isActive && url && (
                <div className="p-3 bg-gagarin-dark rounded-lg mb-3">
      ...
      <div className="card-dark">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
          <Link to={ROUTES.PRICING}
            className="p-4 rounded-xl bg-white/5 border border-gagarin-border hover:border-gagarin-neon/30 transition-colors text-center">
            <Key className="w-5 h-5 text-gagarin-neon mx-auto mb-2" />
          ...
          <a href={TELEGRAM.SUPPORT_URL} target="_blank" rel="noopener noreferrer"
            className="p-4 rounded-xl bg-white/5 border border-gagarin-border hover:border-gagarin-neon/30 transition-colors text-center">
            <Users className="w-5 h-5 text-gagarin-neon mx-auto mb-2" />
      ...
      {activeKeys.map((slot) => {
        ...
        return (
          <div key={slot.key} className="card-dark">
            ...
            <div className="p-3 bg-gagarin-dark rounded-lg">
              <code className="text-xs text-gray-400 break-all">{url}</code>
            </div>
          </div>
        );
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        ...
        <div className="card-dark text-center">
          <div className="text-3xl font-bold text-gagarin-green mb-1">+{(ref?.paid_count || 0) * 7}</div>
      ...
      <div className="card-dark">
        ...
        <div className="space-y-3 text-sm text-gray-400">
          ...
          <p>3. Вы получаете <span className="text-gagarin-green font-semibold">+7 дней</span> к вашей подписке {PRO_SUBSCRIPTION_LABEL}</p>
        </div>
      </div>
      ...
      {ref?.referral_link && (
        <div className="card-dark">
          ...
          <div className="flex items-center gap-2 mb-4">
            <div className="flex-1 p-3 bg-gagarin-dark rounded-lg">
              <code className="text-xs text-gray-400 break-all">{ref.referral_link}</code>
            </div>
            <button
              onClick={copyLink}
              className="p-3 rounded-lg bg-gagarin-neon/10 hover:bg-gagarin-neon/20 transition-colors flex-shrink-0"
            >
              {copied ? <Check className="w-5 h-5 text-gagarin-green" /> : <Copy className="w-5 h-5 text-gagarin-neon" />}
            </button>
          </div>
  ...
  const linkAccount = async () => {
    ...
    try {
      ...
      if (data.success) {
        ...
        localStorage.removeItem('gagarin_user');
    ...
    <div className="space-y-6">
      ...
      <div className="card-dark">
        ...
        <div className="space-y-3">
          <div className="flex items-center justify-between p-3 rounded-xl bg-gagarin-dark">
            <div className="flex items-center gap-2">
              <Send className="w-4 h-4 text-blue-400" />
              <span className="text-sm text-gray-400">Telegram</span>
            </div>
            <span className={`text-sm font-medium ${account?.has_telegram ? 'text-gagarin-green' : 'text-gray-500'}`}>
          ...
          <div className="flex items-center justify-between p-3 rounded-xl bg-gagarin-dark">
            ...
            <span className={`text-sm font-medium ${account?.has_email ? 'text-gagarin-green' : 'text-gray-500'}`}>
      ...
      {account?.has_telegram && account?.has_email ? (
        <div className="card-dark">
          <div className="flex items-center gap-2 mb-2">
            <Link2 className="w-5 h-5 text-gagarin-green" />
        ...
        <div className="card-dark">
          <div className="flex items-center gap-2 mb-2">
            <Link2 className="w-5 h-5 text-gagarin-neon" />
          ...
          {account?.auth_type === 'telegram' && !account?.has_email && (
            <>
              ...
              {linkingCode ? (
                <div className="space-y-3">
                  <div className="flex items-center gap-2">
                    <div className="flex-1 p-3 bg-gagarin-dark rounded-lg text-center">
                      <code className="text-base sm:text-xl font-mono text-white tracking-wider sm:tracking-widest">{linkingCode}</code>
                    </div>
                    <button onClick={copyCode}
                      className="p-3 rounded-lg bg-gagarin-neon/10 hover:bg-gagarin-neon/20 transition-colors flex-shrink-0">
                      {codeCopied ? <Check className="w-5 h-5 text-gagarin-green" /> : <Copy className="w-5 h-5 text-gagarin-neon" />}
                    </button>
                  </div>
            ...
            <>
              ...
              <div className="flex gap-2">
                <input
                  ...
                  className="flex-1 px-3 sm:px-4 py-3 rounded-xl bg-gagarin-dark border border-gagarin-border text-white text-center text-base sm:text-lg tracking-wider sm:tracking-widest focus:border-gagarin-neon focus:outline-none transition-colors font-mono"
