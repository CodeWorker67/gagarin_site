...
const useAuthStore = create((set, get) => ({
  ...
  loadFromStorage: async () => {
    const userJson = localStorage.getItem('gagarin_user');
    if (userJson) {
      try {
        ...
        localStorage.setItem('gagarin_user', JSON.stringify(data));
      } catch {
  ...
  _setAuth: (user, token) => {
    localStorage.setItem('gagarin_user', JSON.stringify(user));
    if (token) {
